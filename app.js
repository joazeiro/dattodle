...
var answerCountry;
var currentRow;
const apiBaseUrl = 'https://4y1d40cjuh.execute-api.us-east-2.amazonaws.com/api';
var devices;
...
function loadCoasters(query) {
    var data = [];
	devices.forEach((element)=> {
		if(element.deviceModel.toLowerCase().includes(query.toLowerCase())){
			data.push(element);
        }
    })
	
    if(data.length > 0) displayCoasterList(data);
}
    ...
    for(let idx = 0; idx < coasters.length; idx++) {
        ...
        coasterListItem.innerHTML = `
            <div class = "search-item-info">
                <h3>${coasters[idx].deviceModel}</h3>
            </div>
        `;
...
async function searchById(id) {
    const result = await fetch(`${apiBaseUrl}/coasters/${id}`);
    const guess = await result.json();
    console.log(guess);
}
...
async function startup() {
    ...
    /* Hardcoded Fixes for Possible Answers */
